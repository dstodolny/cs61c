...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
  ...
  while (fgets(buf, BUF_SIZE, input)) {
    ...
    if (exit_code == 0) {
      ins_count = write_pass_one(output, name, args, num_args);

      if (ins_count == 0)
        exit_code = -1;

      byte_offset += (4 * ins_count);
    }
    ln++;
    num_args = 0;
  }
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
  ...
  while(fgets(buf, BUF_SIZE, input)) {
    ...
    if (translate_inst(output, name, args, num_args, addr, symtbl, reltbl) == -1) {
      raise_inst_error(ln, name, args, num_args);
      exit_code = -1;
    }
    ln ++;
    addr += 4;
